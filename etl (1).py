...
def process_song_file(cur, filepath):
    ...
    rtist_data = artist_data = df[['artist_id','artist_name', 'artist_location', 'artist_latitude', 'artist_longitude']].values[0].tolist()
...
def process_log_file(cur, filepath):
    ...
    time_data = time_data = (t, t.dt.hour, t.dt.day, t.dt.week, t.dt.month, t.dt.year, df.week_day)
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (str(row.start_time), row.userId, row.level, artistid, songid, row.sessionId,row.location,row.userAgent)
